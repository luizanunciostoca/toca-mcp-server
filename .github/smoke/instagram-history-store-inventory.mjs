...
try {
  ...
  for (const row of tables.rows) {
    const schema = String(row.table_schema).replaceAll('"', '""');
    ...
    counts.push({ tableSchema: row.table_schema, tableName: row.table_name, rowCount: result.rows[0]?.count ?? '0' });
  }

  console.log(JSON.stringify({
    validation: 'instagram-history-store-inventory',
    candidateTables: tables.rows.map((row) => ({ tableSchema: row.table_schema, tableName: row.table_name })),
    matchingColumns: columns.rows.map((row) => ({
