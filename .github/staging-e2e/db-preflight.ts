...
try {
  ...
  assert.deepEqual(
    databaseMigrations,
    candidateMigrations,
    "STAGING_E2E_DATABASE_MIGRATION_DRIFT",
  );
  ...
  const claimableWorkflowStepCount = Number(
    claimableWorkflowRows.rows[0]?.count ?? "0",
  );
  assert.equal(
    claimableWorkflowStepCount,
    0,
    "STAGING_E2E_WORKFLOW_QUEUE_NOT_QUIESCENT",
  );
