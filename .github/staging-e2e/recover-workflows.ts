...
function recoverableSynthetic(row: ClaimableRow): boolean {
  return (
    row.tenant_id === "m12-tenant" &&
    row.definition_id === "m-found-12-postgres-restart"
  );
}
...
try {
  ...
  assert.deepEqual(
    unexpected,
    [],
    "STAGING_E2E_UNEXPECTED_CLAIMABLE_WORKFLOW_FAIL_CLOSED",
  );
  ...
  for (const row of result.rows.filter(
    (entry) => entry.step_status === "RUNNING",
  )) {
    assert(
      row.claim_execution_id,
      "STAGING_E2E_RUNNING_STEP_WITHOUT_EXECUTION_ID",
    );
  ...
  for (const workflowId of readyWorkflowIds) {
    const expected = result.rows.filter(
      (entry) =>
        entry.workflow_id === workflowId && entry.step_status === "READY",
    );
    ...
    assert.equal(
      claims.length,
      expected.length,
      "STAGING_E2E_SYNTHETIC_RECOVERY_CLAIM_MISMATCH",
    );
  ...
  assert.equal(
    remainingClaimable,
    0,
    "STAGING_E2E_SYNTHETIC_WORKFLOW_RECOVERY_INCOMPLETE",
  );
