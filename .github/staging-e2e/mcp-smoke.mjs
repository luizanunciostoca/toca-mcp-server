...
try {
  ...
  assert(names.includes('toca.system.health'), 'STAGING_E2E_HEALTH_TOOL_MISSING');
  assert(names.includes('toca.capabilities.search'), 'STAGING_E2E_CAPABILITY_SEARCH_TOOL_MISSING');

  const health = await client.callTool({ name: 'toca.system.health', arguments: {} });
  ...
  await writeFile(join(evidenceDir, 'mcp-protocol-smoke.json'), `${JSON.stringify(summary, null, 2)}\n`);
