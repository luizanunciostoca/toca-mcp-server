...
try {
  const before = await pool.query<{ tenant_id: string; status: string; count: string }>(
  ...
  await writeFile(join(evidenceDir, 'outbox-settlement.json'), `${JSON.stringify(summary, null, 2)}\n`);
