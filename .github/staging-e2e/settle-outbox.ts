...
try {
  ...
  const unexpected = before.rows.filter(
    (row) => !allowedSyntheticTenant(row.tenant_id),
  );
  assert.deepEqual(
    unexpected,
    [],
    "STAGING_E2E_UNEXPECTED_OUTBOX_ROW_FAIL_CLOSED",
  );
  ...
  assert.equal(
    ...
    0,
    "STAGING_E2E_OUTBOX_SETTLEMENT_INCOMPLETE",
  );
