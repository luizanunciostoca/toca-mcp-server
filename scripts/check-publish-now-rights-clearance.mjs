...
if (command.action !== 'PUBLISH_NOW') {
  console.log(
    `PUBLISH_NOW_RIGHTS_CLEARANCE=SKIP action=${String(command.action)}`,
  );
  process.exit(0);
}
...
failUnless(
  clearance && typeof clearance === 'object',
...
const hasAuthority =
  typeof clearance.authority === 'string' &&
  clearance.authority.trim().length > 0;
...
const hasValidClearanceTimestamp =
  typeof clearedAt === 'string' && Number.isFinite(Date.parse(clearedAt));
...
if (clearance.expiresAt !== undefined) {
  ...
  failUnless(Date.parse(expiresAt) > Date.now(), 'RIGHTS_CLEARANCE_EXPIRED');
}
