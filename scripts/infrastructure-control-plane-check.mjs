...
for (const forbidden of forbiddenWorkflowMarkers) {
  if (workflow.includes(forbidden)) {
    console.error(
      `Infrastructure workflow contains forbidden/superseded capability: ${forbidden}`,
    );
    process.exit(1);
  }
}
...
const publicationBucket =
  policy.allowedOperations?.["reconcile-publication-assets-bucket"];
...
const stagingReaders =
  policy.allowedOperations?.["grant-staging-verification-readers"];
...
if (
  ...
  console.error(
    "Staging verification readers are outside the approved envelope",
  );
  process.exit(1);
}

const cloudSqlOptimization =
  policy.allowedOperations?.["optimize-cloud-sql-cost"];
...
if (
  ...
  console.error(
    "Cloud SQL cost optimization is outside the approved resumable envelope",
  );
  process.exit(1);
}
...
if (policy.allowedOperations?.["reconcile-toca-managed-instagram-heartbeat"]) {
  console.error(
    "Legacy heartbeat must not remain an active infrastructure operation",
  );
  process.exit(1);
}
...
if (
  ...
  console.error(
    "TOCA-managed Instagram scheduler topology is outside the minimum-cost envelope",
  );
  process.exit(1);
}
...
if (
  ...
  console.error(
    "Minimum-cost principles changed outside the approved envelope",
  );
  process.exit(1);
}
...
for (const marker of daemonRequirements) {
  if (!daemonWorkflow.includes(marker)) {
    console.error(
      `Daemon deploy workflow missing minimum-cost topology marker: ${marker}`,
    );
    process.exit(1);
  }
}
...
for (const forbidden of [
  ...
  if (daemonWorkflow.includes(forbidden)) {
    console.error(
      `Daemon deploy workflow retains always-on cost marker: ${forbidden}`,
    );
    process.exit(1);
  }
}
...
if (
  ...
  console.error(
    "Daemon source must not retain background polling when scale-to-zero is active",
  );
  process.exit(1);
}
...
for (const permission of requiredStoragePermissions) {
  if (!storageRole.includes(`- ${permission}`)) {
    console.error(
      `Infrastructure storage role missing permission: ${permission}`,
    );
    process.exit(1);
  }
}
...
for (const permission of requiredCloudSqlPermissions) {
  if (!cloudSqlRole.includes(`- ${permission}`)) {
    console.error(
      `Cloud SQL cost optimizer role missing permission: ${permission}`,
    );
    process.exit(1);
  }
}
...
for (const permission of forbiddenStoragePermissions) {
  if (storageRole.includes(permission)) {
    console.error(
      `Infrastructure storage role contains forbidden permission: ${permission}`,
    );
    process.exit(1);
  }
}
...
for (const permission of forbiddenCloudSqlPermissions) {
  if (cloudSqlRole.includes(permission)) {
    console.error(
      `Cloud SQL cost optimizer role contains forbidden permission: ${permission}`,
    );
    process.exit(1);
  }
}
