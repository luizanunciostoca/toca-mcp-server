...
create table if not exists tenant_campaign_scopes (
  ...
  foreign key (tenant_id, provider_id, connected_account_id)
    references tenant_provider_bindings (tenant_id, provider_id, connected_account_id)
    on delete restrict,
  check (length(trim(provider_id)) > 0),
  check (length(trim(connected_account_id)) > 0),
...
-- Existing canonical top-level records already carrying tenant_id are preserved:
-- CRM (012), workflow (007), EventRecord (011), attribution/measurement (013),
-- Privacy (014) and content/creative lineage (020).
-- Canonical CRM Conversation/Message from PR #22 also uses composite tenant/workspace/org FKs.

-- Legacy V1 side-effect roots receive explicit tenant ownership without changing V1 behavior.
...
-- while PostgreSQL enforces tenant_id for every new or modified row after this migration.
...
-- PR #21 owns AG-01 orchestration persistence. If present, prevent an AG-01 MessageRecord
-- from referencing another tenant's orchestration conversation.
do $$
begin
...
-- PR #18 owns Asset Intelligence. If present, tenant-bind source/master lineage so an asset
-- cannot resolve lineage from another tenant.
do $$
begin
