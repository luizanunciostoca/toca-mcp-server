...
function buildEligibleQueueItem(
  ...
  if (creativeTruthBinding.outputSha256.toLowerCase() !== finalAssetSha256) {
    throw new Error(`GITHUB_NATIVE_QUEUE_SYNC_CREATIVE_TRUTH_HASH_MISMATCH:${candidate.contentItemId}`);
  }
...
function requireField<T>(value: T | undefined, contentItemId: string, field: string): T {
