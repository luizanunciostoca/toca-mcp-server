...
export function createRuntimeReadinessChecks(
  ...
  return [
    ...
    namedCheck('audit', async () => {
      const mismatch = await requiredPool(options.pool).query(
    ...
    namedCheck('outbox', async () => {
      ...
      const result = await requiredPool(options.pool).query<{
    ...
    namedCheck('privacy', async () => {
      await requiredPool(options.pool).query(
        'select tenant_id, workspace_id, organization_id from privacy_ledger_events limit 0',
      );
    }),
    namedCheck('workflow', async () => {
      const pool = requiredPool(options.pool);
      await Promise.all([
        pool.query(
    ...
    namedCheck('critical_configuration', async () => {
      ...
      const role = requireServiceRole(env);
      if (role === 'mcp' && !options.config.MCP_ENABLED) {
        throw new Error('READINESS_MCP_ROLE_REQUIRES_MCP_ENABLED');
      }
...
function requireReferencedSecret(
  ...
  if (!env[normalizedKey]?.trim()) throw new Error(`READINESS_SECRET_MISSING:${source}`);
}

function requireServiceRole(env: NodeJS.ProcessEnv): 'mcp' | 'webhook' {
  const role = requireText(env.TOCA_SERVICE_ROLE, 'READINESS_SERVICE_ROLE_REQUIRED');
  if (role !== 'mcp' && role !== 'webhook') throw new Error('READINESS_SERVICE_ROLE_INVALID');
  return role;
}
