...
export class OpenAiSceneContinuationVideoProvider {
  ...
  async generate(request: SceneContinuationVideoRequest): Promise<SceneContinuationVideoResult> {
    ...
    const sourceBuffer = Uint8Array.from(request.source.bytes).buffer;
    form.set(
      'input_reference',
    ...
    for (let attempt = 0; attempt < this.maxPolls; attempt += 1) {
      const response = await this.fetchImpl(
        `${OPENAI_VIDEOS_ENDPOINT}/${encodeURIComponent(videoId)}`,
        {
          method: 'GET',
          headers: { Authorization: `Bearer ${apiKey}` },
        },
    ...
    if (!completed) {
      throw new ExecutionError(
        'PROVIDER_UNAVAILABLE',
        'OPENAI_SCENE_CONTINUATION_VIDEO_TIMEOUT',
        true,
      );
    }
    ...
    if (!isMp4(outputBytes)) {
      throw new ExecutionError(
        ...
        'OPENAI_VIDEO_OUTPUT_INVALID_MP4',
        false,
      );
    }
