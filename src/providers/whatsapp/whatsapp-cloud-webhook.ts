...
function normalizeStatus(input: {
  ...
  const errors = arrayValue(input.raw.errors).map(asRecord);
...
function normalizeMessageContent(
  ...
  if (providerType === 'text') {
    const body = stringValue(asRecord(raw.text).body) ?? null;
    return { text: body, attachments: [], payload: {} };
  }
...
function toContentType(value: string): CrmMessageContentType {
  const mapping: Readonly<Record<string, CrmMessageContentType>> = {
    text: 'TEXT',
    image: 'IMAGE',
    audio: 'AUDIO',
    video: 'VIDEO',
...
function toDeliveryStatus(value: string | undefined): CrmDeliveryStatus | undefined {
