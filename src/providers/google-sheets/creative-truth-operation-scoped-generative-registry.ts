...
import { TOCA_CREATIVE_TRUTH_POLICY_ID, type VenueAsset, type VenueReference } from '../../contracts/creative-truth.js';
  ...
  implements OperationScopedGenerativeRegistry
{
  ...
  async assertCanonicalPolicy(): Promise<void> {
    const [policyRows, referenceRows] = await Promise.all([
      ...
      this.base.assertCanonicalPolicy(),
    ]).then(([policyRows, referenceRows]) => [policyRows, referenceRows] as const);
