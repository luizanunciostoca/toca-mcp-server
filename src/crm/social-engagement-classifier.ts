...
export function classifySocialEngagement(text: string): SocialEngagementClassification {
  ...
  const refund = hasAny(normalized, [
    'reembolso', 'estorno', 'refund', 'devolucao', 'cobranca', 'cobrado duas vezes',
    'cobranca duplicada', 'chargeback',
  ]);
  const harassmentOrThreat = hasAny(normalized, [
    'ameaca', 'ameacando', 'assedio', 'assediado', 'assediada', 'harassment', 'threat',
    'threatening',
  ]);
  const legal = hasAny(normalized, [
    'advogado', 'processo', 'justica', 'boletim de ocorrencia', 'legal action', 'lawsuit',
  ]);
  const safety = hasAny(normalized, [
    'acidente', 'agressao', 'seguranca', 'machucado', 'machucada', 'emergencia', 'emergency',
    'assault',
  ]);
  const press = hasAny(normalized, [
    'imprensa', 'jornalista', 'reportagem', 'press inquiry', 'media request',
  ]);
  const partnership = hasAny(normalized, [
    'parceria', 'collab', 'colaboracao', 'patrocinio', 'fornecedor', 'agencia', 'influencer',
    'criador de conteudo', 'artista', 'partnership', 'collaboration', 'sponsorship',
  ]);
  const publicFigure = hasAny(normalized, [
    'vip', 'celebridade', 'famoso', 'famosa', 'artista conhecido', 'public figure', 'celebrity',
  ]);
  const careers = hasAny(normalized, [
    'trabalhe conosco', 'vaga', 'curriculo', 'emprego', 'contratando', 'job opening', 'resume',
    'work with you',
  ]);
  const complaint = hasAny(normalized, [
    'reclamacao', 'reclamar', 'pessimo', 'horrivel', 'problema serio', 'complaint', 'terrible',
  ]);
  const support = complaint || refund || hasAny(normalized, [
    'preciso de ajuda', 'nao funcionou', 'deu erro', 'problema com', 'suporte', 'help me',
    'not working', 'support',
  ]);
  ...
  const gastronomy = hasAny(normalized, [
    'cardapio', 'menu', 'comida', 'prato', 'petisco', 'gastronomia', 'drink', 'bebida', 'cerveja',
    'cocktail', 'food',
  ]);
  const praise = hasAny(normalized, [
    'amei', 'adorei', 'maravilhoso', 'incrivel', 'parabens', 'melhor experiencia', 'love it',
    'amazing', 'congratulations',
  ]);
  const ugc = hasAny(normalized, [
    '@tocadomorcego', 'marquei voces', 'marquei vcs', 'repost', 'repostar', 'marcacao',
    'tagged you', 'mentioned you',
  ]);
  const spam = hasAny(normalized, [
    'ganhe seguidores', 'compre seguidores', 'divulgue seu perfil', 'bitcoin investment',
    'crypto investment', 'seo service', 'social media growth service',
  ]);
  const abusiveLanguage = hasAny(normalized, [
    'vou acabar com voces', 'vou pegar voces', 'matar voces', 'idiotas', 'desgracados',
  ]);
  const locationHours = hasAny(normalized, [
    'horario', 'que horas', 'onde fica', 'localizacao', 'como chegar', 'quando', 'abre', 'fecha',
    'comeca', 'termina', 'funciona todos os dias', 'hours', 'what time', 'where is', 'how to get',
    'when', 'opens', 'closes', 'starts', 'ends',
  ]);
  const operationalFaq = hasAny(normalized, [
    'idade minima', 'menor de idade', 'dress code', 'traje', 'acessibilidade', 'cadeirante',
    'estacionamento', 'forma de pagamento', 'aceita cartao', 'pode entrar com',
  ...
  const buyingSignal = hasAny(normalized, [
    'comprar', 'quero ir', 'quero ingresso', 'garantir', 'disponibilidade', 'buy', 'purchase',
    'i want to go', 'available',
  ]);
  ...
  const knownLowOrCommercial =
    ...
    gastronomy || praise || ugc || partnership || careers || support || spam;
  const materialUnknown = !knownLowOrCommercial && isMaterialQuestion(normalized);

  const intent = harassmentOrThreat || abusiveLanguage
  ...
  const urgency = classifyUrgency(normalized, safety || harassmentOrThreat || abusiveLanguage, refund);
  ...
  const confidence = classifyConfidence({
    ...
    spam,
    highSpecificity:
      harassmentOrThreat || abusiveLanguage || legal || safety || refund || complaint || support ||
      ticket || reservation || price || locationHours || operationalFaq || gastronomy || partnership ||
      careers || praise || ugc,
    eventInterest,
  });
...
function classifyConversationIntents(input: {
  ...
  if (input.safety || input.legal || input.support) add('SUPPORT');
...
function classifyPriority(input: {
  ...
  if (input.harassmentOrThreat || input.safety || input.legal || input.urgency === 'CRITICAL') return 'P0';
  if (
    input.containsPotentialSensitiveData || input.complaint || input.refund || input.support ||
    input.commercialIntent === 'HIGH' || input.urgency === 'HIGH'
  ) return 'P1';
...
function isMaterialQuestion(value: string): boolean {
  ...
  return value.includes('?') || hasAny(` ${value} `, [
    ' como ', ' quando ', ' onde ', ' qual ', ' quais ', ' quanto ', ' quem ', ' posso ', ' pode ',
    ' tem ', ' existe ', ' how ', ' when ', ' where ', ' what ', ' who ', ' can ', ' do you ',
    ' hay ', ' donde ', ' cuando ', ' cuanto ',
  ]);
}
...
function classifySentiment(value: string): SocialSentiment {
  if (hasAny(value, ['amei', 'incrivel', 'maravilhoso', 'top', 'love', 'amazing', 'great'])) return 'POSITIVE';
  if (hasAny(value, ['pessimo', 'horrivel', 'raiva', 'odio', 'terrible', 'awful', 'angry'])) return 'NEGATIVE';
  return 'NEUTRAL';
}
...
function classifyLanguage(value: string): SocialLanguage {
  const portuguese = hasAny(value, [' quero ', ' preco', ' ingresso', ' hoje', ' onde ', 'horario']);
  const spanish = hasAny(value, ['quiero', 'precio', 'entrada', ' donde ', 'horario']);
  const english = hasAny(value, [' the ', ' want ', ' price', ' ticket', ' today', ' where ', ' how ']);
