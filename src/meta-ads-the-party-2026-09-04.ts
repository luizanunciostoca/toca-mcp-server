...
} from './providers/meta-ads/meta-ads-the-party-2026-09-04-plan.js';
import { createMetaPublicationApiClient } from './providers/meta/meta-publication-client.js';

const config = loadConfig(process.env);
const mode = requiredEnv('META_ADS_THE_PARTY_0904_MODE');
const explicitApproval = requiredEnv('META_ADS_THE_PARTY_0904_EXPLICIT_APPROVAL');
if (explicitApproval !== THE_PARTY_2026_09_04_APPROVAL) {
  throw new Error('META_ADS_THE_PARTY_0904_EXPLICIT_APPROVAL_MISMATCH');
}

const api = createMetaPublicationApiClient(config);
const provider = new MetaAdsControlledGraphProvider(api);
const readProvider = new MetaAdsReadProvider(api);

if (mode === 'PREPARE') {
  const prepared = await prepare();
  console.log(`META_ADS_THE_PARTY_0904_PREPARE_RESULT=${JSON.stringify(prepared)}`);
} else if (mode === 'EXECUTE_PAUSED') {
  const executed = await executePaused();
  console.log(`META_ADS_THE_PARTY_0904_EXECUTE_RESULT=${JSON.stringify(executed)}`);
} else {
  throw new Error('META_ADS_THE_PARTY_0904_MODE_UNSUPPORTED');
}

async function prepare(): Promise<Readonly<Record<string, unknown>>> {
  const descriptor = buildTheParty20260904Descriptor();
  assertExactTheParty20260904Descriptor(descriptor);
  ...
  const deliveryEstimate = await verifyDeliveryEstimate(descriptor);
  const requestSha256 = theParty20260904DescriptorSha256(descriptor);

  return {
    status: 'READY_FOR_EXACT_CREATE_PAUSED',
...
async function executePaused(): Promise<Readonly<Record<string, unknown>>> {
  ...
  const approvedSha256 = requiredEnv('META_ADS_THE_PARTY_0904_APPROVED_SHA256');
  const descriptorBase64 = requiredEnv('META_ADS_THE_PARTY_0904_DESCRIPTOR_B64');
  ...
  assertExactTheParty20260904Descriptor(descriptor);
  assertCampaignStartStillFuture(descriptor);
  const computedSha256 = theParty20260904DescriptorSha256(descriptor);
  if (computedSha256 !== approvedSha256) {
    throw new Error('META_ADS_THE_PARTY_0904_APPROVED_SHA256_MISMATCH');
  }
  ...
  const correlationId = `meta-ads:the-party:2026-09-04:create-paused:${approvedSha256}`;
  ...
  await pool.query(
    ...
    [
      ...
      'meta_ads.the_party_2026_09_04.create_paused',
      'WRITE_EXTERNAL',
      'APPROVED_CAMPAIGN_CREATE_PAUSED_STARTED',
      JSON.stringify({
        eventId: descriptor.eventId,
  ...
  try {
    ...
    for (const asset of assets) {
      imageHashes.push(await uploadImage(descriptor, asset.base64));
    }
    ...
    for (const [index, asset] of descriptor.assets.entries()) {
      ...
      if (!imageHash) throw new Error('META_ADS_THE_PARTY_0904_IMAGE_HASH_MISSING');
    ...
    for (const [index, asset] of descriptor.assets.entries()) {
      ...
      if (!creativeId) throw new Error('META_ADS_THE_PARTY_0904_CREATIVE_ID_MISSING');
    ...
    assertConfiguredPaused(pausedVerification);
    ...
    await pool.query(
      ...
      [
        ...
        'meta_ads.the_party_2026_09_04.create_paused',
        'WRITE_EXTERNAL',
        'APPROVED_CAMPAIGN_CREATE_PAUSED_SUCCEEDED',
        JSON.stringify({
          eventId: descriptor.eventId,
          requestSha256: approvedSha256,
          lifetimeBudgetMinor: descriptor.adSet.lifetimeBudgetMinor,
        ...
        JSON.stringify({
          ...
          imageHashes,
          deliveryEstimate,
          providerVerification: pausedVerification,
        }),
      ],
    );

    return {
      status: 'PAUSED_READY_FOR_MANUAL_ACTIVATION',
      eventId: descriptor.eventId,
      requestSha256: approvedSha256,
      campaignId: campaign.id,
      adSetId: adSet.id,
    ...
    await pool.query(
      ...
      [
        ...
        'meta_ads.the_party_2026_09_04.create_paused',
        'WRITE_EXTERNAL',
        'APPROVED_CAMPAIGN_CREATE_PAUSED_FAILED_PAUSED_ROLLBACK',
...
async function loadAndVerifyAssets(
  ...
  for (const asset of descriptor.assets) {
    const path = `ops/meta-ads/the-party-2026-09-04/${asset.fileName}`;
    const bytes = await readFile(path);
    const base64 = bytes.toString('base64');
    if (bytes.length < 100_000 || bytes[0] !== 0xff || bytes[1] !== 0xd8) {
      throw new Error(`META_ADS_THE_PARTY_0904_ASSET_INVALID_${asset.key}`);
    }
    const sourceSha256 = createHash('sha256').update(bytes).digest('hex');
    if (sourceSha256 !== asset.sourceSha256) {
      throw new Error(`META_ADS_THE_PARTY_0904_ASSET_SHA256_MISMATCH_${asset.key}`);
    }
...
async function verifyPermissions(): Promise<readonly string[]> {
  ...
  if (!granted.includes('ads_management')) {
    throw new Error('META_ADS_THE_PARTY_0904_ADS_MANAGEMENT_REQUIRED');
  }
  return granted;
}
...
async function verifyAccount(
  ...
  if (!scalarString(account.id).endsWith(descriptor.account.adAccountId)) {
    throw new Error('META_ADS_THE_PARTY_0904_ACCOUNT_ID_MISMATCH');
  }
  if (scalarString(account.currency) !== descriptor.account.currency) {
    throw new Error('META_ADS_THE_PARTY_0904_CURRENCY_MISMATCH');
  }
  if (finiteNumber(account.account_status) !== 1) {
    throw new Error('META_ADS_THE_PARTY_0904_ACCOUNT_NOT_ACTIVE');
  }
  return account;
}

async function verifyDeliveryEstimate(
  descriptor: TheParty20260904CampaignDescriptor,
): Promise<Readonly<Record<string, unknown>>> {
  const estimate = await readProvider.getDeliveryEstimate(descriptor.account, {
    optimizationGoal: 'REACH',
    targetingSpec: descriptor.adSet.targeting,
  });
  if (!estimate.estimateReady) {
    throw new Error('META_ADS_THE_PARTY_0904_DELIVERY_ESTIMATE_NOT_READY');
  }
  if (estimate.upperBound <= 0) {
    throw new Error('META_ADS_THE_PARTY_0904_DELIVERY_ESTIMATE_EMPTY');
  }
  return {
...
async function assertNoDuplicateCampaign(
  ...
  if (campaigns.some((campaign) => scalarString(campaign.name) === descriptor.campaign.name)) {
    throw new Error('META_ADS_THE_PARTY_0904_DUPLICATE_CAMPAIGN_NAME');
  }
}
...
async function uploadImage(
  ...
  const response = asRecord(
    await api.post(`act_${descriptor.account.adAccountId}/adimages`, {
      bytes: base64,
    }),
  );
  ...
  throw new Error('META_ADS_THE_PARTY_0904_IMAGE_UPLOAD_HASH_NOT_RETURNED');
}
...
async function verifyProviderEnvelope(
  ...
  if (!created.campaignId || !created.adSetId) {
    throw new Error('META_ADS_THE_PARTY_0904_CREATED_OBJECTS_INCOMPLETE');
  }
  if (created.creativeIds.length !== 5 || created.adIds.length !== 5 || imageHashes.length !== 5) {
    throw new Error('META_ADS_THE_PARTY_0904_CREATED_COUNTS_MISMATCH');
  }
  ...
  if (scalarString(campaign.name) !== descriptor.campaign.name) {
    throw new Error('META_ADS_THE_PARTY_0904_PROVIDER_CAMPAIGN_NAME_MISMATCH');
  }
  if (scalarString(campaign.objective) !== descriptor.campaign.objective) {
    throw new Error('META_ADS_THE_PARTY_0904_PROVIDER_OBJECTIVE_MISMATCH');
  }
  ...
  for (const [index, creative] of creatives.entries()) {
    ...
    if (!expected || !expectedHash) {
      throw new Error('META_ADS_THE_PARTY_0904_PROVIDER_CREATIVE_INDEX');
    }
    if (scalarString(creative.id) !== created.creativeIds[index]) {
      throw new Error('META_ADS_THE_PARTY_0904_PROVIDER_CREATIVE_ID_MISMATCH');
    }
    if (scalarString(creative.name) !== expected.creativeName) {
      throw new Error('META_ADS_THE_PARTY_0904_PROVIDER_CREATIVE_NAME_MISMATCH');
    }
    const spec = asRecord(creative.object_story_spec);
    const linkData = asRecord(spec.link_data);
    if (scalarString(linkData.link) !== expected.destinationUrl) {
      throw new Error('META_ADS_THE_PARTY_0904_PROVIDER_DESTINATION_MISMATCH');
    }
    const returnedHash = scalarString(linkData.image_hash);
    if (returnedHash && returnedHash !== expectedHash) {
      throw new Error('META_ADS_THE_PARTY_0904_PROVIDER_IMAGE_HASH_MISMATCH');
    }
  }

  for (const [index, ad] of ads.entries()) {
    const expected = descriptor.assets[index];
    if (!expected) throw new Error('META_ADS_THE_PARTY_0904_PROVIDER_AD_INDEX');
    if (scalarString(ad.id) !== created.adIds[index] || scalarString(ad.name) !== expected.adName) {
      throw new Error('META_ADS_THE_PARTY_0904_PROVIDER_AD_MISMATCH');
    }
  }
...
function assertAdSetEnvelope(
  ...
  if (scalarString(adSet.name) !== descriptor.adSet.name) {
    throw new Error('META_ADS_THE_PARTY_0904_PROVIDER_ADSET_NAME_MISMATCH');
  }
  if (finiteNumber(adSet.lifetime_budget) !== descriptor.adSet.lifetimeBudgetMinor) {
    throw new Error('META_ADS_THE_PARTY_0904_PROVIDER_TOTAL_BUDGET_MISMATCH');
  }
  if (scalarString(adSet.optimization_goal) !== descriptor.adSet.optimizationGoal) {
    throw new Error('META_ADS_THE_PARTY_0904_PROVIDER_OPTIMIZATION_MISMATCH');
  }
  if (scalarString(adSet.billing_event) !== descriptor.adSet.billingEvent) {
    throw new Error('META_ADS_THE_PARTY_0904_PROVIDER_BILLING_MISMATCH');
  }
  ...
  if (platforms.join(',') !== 'facebook,instagram') {
    throw new Error('META_ADS_THE_PARTY_0904_PROVIDER_PLATFORMS_MISMATCH');
  }
  ...
  if (!location || locations.length !== 1) {
    throw new Error('META_ADS_THE_PARTY_0904_PROVIDER_GEO_COUNT_MISMATCH');
  }
  if (
    ...
    throw new Error('META_ADS_THE_PARTY_0904_PROVIDER_GEO_MISMATCH');
  }
}

function assertConfiguredPaused(verification: {
  readonly campaign: Readonly<Record<string, unknown>>;
  readonly adSet: Readonly<Record<string, unknown>>;
  readonly ads: readonly Readonly<Record<string, unknown>>[];
}): void {
  if (scalarString(verification.campaign.status) !== 'PAUSED') {
    throw new Error('META_ADS_THE_PARTY_0904_CAMPAIGN_NOT_PAUSED');
  }
  if (scalarString(verification.adSet.status) !== 'PAUSED') {
    throw new Error('META_ADS_THE_PARTY_0904_ADSET_NOT_PAUSED');
  }
  if (verification.ads.some((ad) => scalarString(ad.status) !== 'PAUSED')) {
    throw new Error('META_ADS_THE_PARTY_0904_AD_NOT_PAUSED');
  }
}
...
function assertSameInstant(actual: unknown, expected: string, label: string): void {
  ...
  if (!Number.isFinite(actualMs) || actualMs !== expectedMs) {
    throw new Error(`META_ADS_THE_PARTY_0904_PROVIDER_${label}_TIME_MISMATCH`);
  }
}
...
function assertCampaignStartStillFuture(descriptor: TheParty20260904CampaignDescriptor): void {
  const startTime = new Date(descriptor.adSet.startTime).getTime();
  if (!Number.isFinite(startTime) || Date.now() >= startTime) {
    throw new Error('META_ADS_THE_PARTY_0904_START_ALREADY_REACHED');
  }
}
