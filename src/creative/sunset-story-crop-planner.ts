import type { NormalizedRect, SunsetStoryImageProfile, SunsetStoryZone } from './sunset-story-image-profile.js';
...
function buildCandidate(
  ...
  const planScore =
    subjectCoverage * 55 + (1 - protectedOverlap) * 30 + subjectPlacementScore * 15;
...
export function planSunsetStoryCrop(
  ...
  const targetZones =
    template.preferredSubjectZones.length > 0 ? template.preferredSubjectZones : (['CENTER'] as const);
