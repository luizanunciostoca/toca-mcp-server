...
function resolveSceneClass(
  ...
  if (primarySubject?.kind === 'SCENERY') return observation.horizonY === null ? 'SCENERY' : 'SEA_VIEW';
  if (observation.horizonY !== null) return 'SEA_VIEW';
  return 'UNKNOWN';
}

function resolveBrightness(regionLuma: Readonly<Partial<Record<SunsetStoryZone, number>>>): SunsetStoryBrightness {
...
export function buildSunsetStoryImageProfile(
  ...
  if (observation.horizonY !== null) assertUnit(observation.horizonY, 'SUNSET_IMAGE_HORIZON_INVALID');
  ...
  const primarySubject =
    observation.subjects.reduce<SunsetStoryObservedSubject | null>(
