...
const NEW_ASSETS = [
  {
    key: 'AD06_DUAS_PISTAS_DUAS_ENERGIAS',
    fileName: 'creative-06-duas-pistas-duas-energias.jpg',
    sourceSha256: '43ce611d214dde861fab41f6a035a1d483ef08835e534c758a3b1dbbc50780bb',
  ...
  {
    key: 'AD07_DUAS_PISTAS_DUAS_EXPERIENCIAS',
    fileName: 'creative-07-duas-pistas-duas-experiencias.jpg',
    sourceSha256: 'c34a823820b3d13fc71f0d194703e454db30e1271d509ed6d8bee2e03f4ece9c',
...
try {
  ...
  for (const [index, asset] of NEW_ASSETS.entries()) {
    ...
    if (!imageHash) throw new Error('META_ADS_THE_PARTY_0904_ADD_ACTIVATE_IMAGE_HASH_MISSING');
...
async function loadAndVerifyAssets(): Promise<readonly VerifiedAsset[]> {
  ...
  for (const asset of NEW_ASSETS) {
    ...
    if (bytes.length < 100_000 || bytes[0] !== 0xff || bytes[1] !== 0xd8) {
      throw new Error(`META_ADS_THE_PARTY_0904_ADD_ACTIVATE_ASSET_INVALID_${asset.key}`);
    }
    const sha256 = createHash('sha256').update(bytes).digest('hex');
    if (sha256 !== asset.sourceSha256) {
      throw new Error(`META_ADS_THE_PARTY_0904_ADD_ACTIVATE_ASSET_SHA256_MISMATCH_${asset.key}`);
    }
  ...
  await pool.query(
    ...
    [
      ...
      JSON.stringify({
        ...
        lifetimeBudgetMinor: EXPECTED_BUDGET_MINOR,
        currency: 'BRL',
        addedCreativeKeys: NEW_ASSETS.map((asset) => asset.key),
      }),
...
function requiredScalar(value: unknown, label: string): string {
  ...
  if (!result) throw new Error(`META_ADS_THE_PARTY_0904_ADD_ACTIVATE_${label}_MISSING`);
  return result;
}
