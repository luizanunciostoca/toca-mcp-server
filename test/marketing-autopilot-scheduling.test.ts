...
import { assertScheduledStateClaim, deriveSchedulingDisposition } from '../src/marketing-autopilot-scheduling.js';

describe('scheduling guards', () => {
  it('rejects SCHEDULED without provider evidence', () => {
    expect(() => assertScheduledStateClaim('SCHEDULED')).toThrow('PROVIDER_SCHEDULE_EVIDENCE_REQUIRED');
  });

  it('accepts provider-confirmed scheduling', () => {
    expect(() => assertScheduledStateClaim('SCHEDULED', {
  ...
  it('uses manual handoff when native scheduling automation is unavailable', () => {
    expect(deriveSchedulingDisposition({
