...
postgresDescribe('DLQ schema-033 remediation PostgreSQL E2E', () => {
  it(
    'serializes concurrent canonical put calls by original job without a new unique constraint',
    async () => {
      ...
      try {
        ...
        await pool.query('delete from dead_letter_jobs where original_job_id = $1', [
          originalJobId,
        ]);
        await pool.end();
      }
  ...
  it(
    'fails closed on missing/cross-tenant ownership and finalizes the owner exactly once',
    async () => {
      const pool = createPostgresPool({ connectionString: databaseUrl(), max: 8 });
      const suffix = `${Date.now()}-${Math.random().toString(16).slice(2)}`;
      ...
      try {
        ...
        await pool.query('update scheduled_jobs set tenant_id = $2 where id = $1', [
          jobId,
          ownerTenant,
        ]);
