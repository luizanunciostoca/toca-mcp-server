...
const corruptBytes = Uint8Array.from([
  0, 0, 0, 20, 0x66, 0x74, 0x79, 0x70, 9, 9, 9, 9,
]);
  ...
  const fetchImpl = vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
    ...
    if (url.startsWith('https://storage.googleapis.com/toca-bucket/instagram/')) {
      ...
      if (headers.get('Range')) {
        ...
        return new Response(Uint8Array.from([videoBytes[0]!]).buffer as ArrayBuffer, {
          status: 206,
...
describe('GcsPhotoToVideoArtifactStore', () => {
  it('stages exact candidate bytes then performs full-SHA durable readback', async () => {
    ...
    expect(gcp.spy.mock.calls.some(([input, init]) =>
      String(input).startsWith('https://storage.googleapis.com/upload/storage/v1/b/toca-bucket/o') &&
      init?.method === 'POST',
    )).toBe(true);
  });
  ...
  it('rejects artifact refs outside the configured bucket before any provider access', async () => {
    ...
    await expect(
      store.loadExact(
        'gcs://other-bucket/instagram/photo-motion-review-v1/file.mp4',
        videoSha256,
      ),
