...
export type MFound12ProviderBehavior =
  'SUCCESS' | 'TIMEOUT_AFTER_ACCEPT' | 'AMBIGUOUS' | 'MISSING_READBACK';
...
export const M_FOUND_12_SCENARIOS: readonly MFound12Scenario[] = [
  ...
  scenario('008', 'TIMER', 'WRITE_REVERSIBLE', true, false, true, 'SUCCESS', true, 'WAITS_DURABLY'),
  scenario(
    '009',
    'RETRY',
    'WRITE_EXTERNAL',
...
export class MFound12FakeProvider {
  ...
  execute(payload: unknown): Promise<Readonly<Record<string, unknown>>> {
    ...
    if (this.behavior === 'TIMEOUT_AFTER_ACCEPT') {
      return Promise.reject(new Error('M_FOUND_12_FAKE_PROVIDER_TIMEOUT_AFTER_ACCEPT'));
    }
    if (this.behavior === 'AMBIGUOUS') {
      return Promise.resolve({ accepted: true, providerState: 'UNKNOWN' });
    }
    return Promise.resolve({ accepted: true, providerId: 'fake-provider-resource-1' });
  }

  readback(): Promise<MFound12FakeProviderReadback> {
    ...
    return Promise.resolve({ verified: true, evidence: ['fake:provider:verified'] });
  }
}
