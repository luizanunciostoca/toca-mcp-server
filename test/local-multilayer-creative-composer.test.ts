...
const registry: ArtistAsset = {
  artistAssetId: 'ARTIST-ILLUSIONIZE-001', artist: 'ILLUSIONIZE', sourceAssetId: 'PRESSKIT-001',
  sourceDriveFileId: 'drive-artist', sourceSha256: sha256Artist(artistBytes), usageScope: ['THE_PARTY'],
  aiModificationAllowed: false, physicalModificationAllowed: false, conventionalTreatmentAllowed: true,
  cropAllowed: true, compositionAllowed: true, protectedElements: ['FACE','HAIR','BODY','CLOTHING'],
...
describe('LocalMultiLayerCreativeComposer', () => {
  it('renders with a protected artist mask and no generative provider', async () => {
    const runner = vi.fn(async (_command: string, args: readonly string[]) => {
      const output = args.at(-1); if (!output) throw new Error('missing output');
    ...
    const result = await composer.compose({
      artist: { assetId: 'PRESSKIT-001', driveFileId: 'drive-artist', bytes: artistBytes, contentType: 'image/jpeg', registry },
      venue: { assetId: 'VENUE-001', driveFileId: 'drive-venue', bytes: venueBytes, contentType: 'image/jpeg' },
      artistProtectionMaskBytes: maskBytes, maskContentType: 'image/png', canvas: '1080x1350', venueOpacityPercent: 55,
      orangeTint: '#d96b16', fadeDirection: 'RIGHT_TO_LEFT',
    });
  ...
  it('fails closed when artist source hash does not match registry', async () => {
    ...
    await expect(composer.compose({
      artist: { assetId: 'PRESSKIT-001', driveFileId: 'drive-artist', bytes: Uint8Array.from([99]), contentType: 'image/jpeg', registry },
      venue: { assetId: 'VENUE-001', driveFileId: 'drive-venue', bytes: venueBytes, contentType: 'image/jpeg' },
      artistProtectionMaskBytes: maskBytes, maskContentType: 'image/png', canvas: '1080x1350',
