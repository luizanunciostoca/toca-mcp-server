...
describe('Privacy / LGPD final operational regressions', () => {
  it(
    'blocks a scheduled follow-up when consent is revoked after the original eligibility decision',
    async () => {
  ...
  it(
    'deduplicates a repeated provider privacy event by execution/capability and preserves one canonical event',
    async () => {
  ...
  it(
    'treats ANONYMIZE retention as destructive and suppresses further purpose use with audit evidence',
    async () => {
      ...
      expect(
        auditSink.list().some((event) => event.executionId === 'retention-anonymize'),
      ).toBe(true);
