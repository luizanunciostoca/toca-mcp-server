...
describe('Instagram Comment canary read-only eligibility gate', () => {
  ...
  it(
    'reports bounded rejection and blocking causes with an explicit safe target marker',
    () => {
      for (const marker of [
  ...
  it(
    'runs the Comment probe once, polls newest logs boundedly and cleans up',
    () => {
  ...
  it(
    'publishes only bounded counts, status and a hashed or explicit NONE target',
    () => {
      for (const marker of [
