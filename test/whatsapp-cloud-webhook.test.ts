...
describe('WhatsApp Cloud webhook parser', () => {
  it('normalizes inbound text, media and delivery status callbacks', () => {
  ...
  it('produces deterministic event ids and deduplicates duplicate provider callbacks in one body', () => {
    ...
    duplicated.entry[0]!.changes[0]!.value.messages.push(
      duplicated.entry[0]!.changes[0]!.value.messages[0],
    );
    expect(parseWhatsAppWebhookEvents(Buffer.from(JSON.stringify(duplicated)))).toHaveLength(4);
  });

  it('ignores non-WhatsApp webhook objects rather than cross-routing Meta events', () => {
    expect(
...
describe('WhatsApp preference and human handoff commands', () => {
  it('recognizes narrow explicit opt-out and opt-in commands', () => {
  ...
  it('recognizes explicit requests for a human without fuzzy substring matching', () => {
