...
describe('Instagram Comment canary opportunity watch heartbeat', () => {
  ...
  it('uses a bounded delay and revalidates before every probe dispatch', () => {
    ...
    expect(workflow).toContain("if [[ \"$(jq -r '.state' <<< \"$ISSUE_JSON\")\" != 'open' ]]");
    expect(workflow).toContain('if [[ "$RUNTIME_SHA" != "$CURRENT_MAIN_SHA" || "$NOW_EPOCH" -ge "$EXPIRES_EPOCH" ]]');
    expect(workflow).toContain("select(.status != \"completed\")");
    expect(workflow).toContain('COMMENT_CANARY_WATCH_HEARTBEAT_DECISION=SKIP_EXISTING_ACTIVE_PROBE');
  });
  ...
  it('dispatches only the canonical read-only watch and never the provider canary', () => {
    ...
    expect(workflow).toContain('TARGET_WORKFLOW=instagram-engagement-comment-canary-opportunity-watch.yml');
  ...
  it('self-rearms only while the same watch remains open, current and unexpired', () => {
    ...
    expect(workflow).toContain('COMMENT_CANARY_WATCH_HEARTBEAT_REARM=SKIPPED_EXISTING_ACTIVE_HEARTBEAT');
    expect(workflow).toContain(
