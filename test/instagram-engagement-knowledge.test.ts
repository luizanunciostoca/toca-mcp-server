...
describe('Instagram engagement knowledge', () => {
    ...
    const client = {
      async readRange() {
        return [
          header,
          [
            'FAQ-001', 'CAT-001', 'Horarios', '', 'Que horas abre?',
            'Qual o horario?;Que horas voces abrem?', '', '', '', '',
            'LOCATION_HOURS', 'LOW', 'AUTO_REPLY_ALLOWED', 'Resposta oficial',
            'TOCA_OS/05_MARKETING', '', '2026-08-28', 'VALIDADO', '',
          ],
        ];
      },
    };
  ...
  it('never verifies a human-review intent even if the sheet is misconfigured', () => {
    const rows = parseKnowledgeRows([
      header,
      [
        'FAQ-HIGH', 'CAT-009', 'Refund', '', 'Quero reembolso', '', '', '', '', '',
        'REFUND', 'HIGH', 'AUTO_REPLY_ALLOWED', 'Resposta indevida', 'fonte', '',
        '2026-08-28', 'VALIDADO', '',
      ],
    ]);
