...
describe('Instagram Comment LIMITED promotion controller', () => {
  it('is owner-only, exact-main and separately authorized', () => {
    for (const marker of [
  ...
  it('requires immutable evidence from the successful recovery Comment canary', () => {
    for (const marker of [
      'CANARY_SOURCE_SHA=',
      'CANARY_RUNTIME_IMAGE_DIGEST=',
      'CANARY_AUTHORIZATION_ISSUE=',
      'CANARY_RUN_ID=',
      'AUTHORIZATION_STATE=CONSUMED_AND_CLOSED',
      'INSTAGRAM_ENGAGEMENT_REAL_COMMENT_CANARY_RECOVERY_60M=AUTHORIZED',
      'CANARY_MAX_EXTERNAL_REPLIES=1',
      'MAX_AGE_MINUTES=60',
      'RECOVERY_ONE_SHOT=true',
      'PERSISTENT_COMMENT_PROMOTION_AUTHORIZED=false',
      '.github/workflows/instagram-engagement-comment-provider-canary-recovery-60m.yml',
      '.conclusion == "success"',
      'COMMENT_CANARY_STATUS=PASS',
      'COMMENT_CANARY_RECOVERY_60M=true',
      'CANARY_SENT_COUNT=1',
      'PROVIDER_ACKNOWLEDGED=true',
      'CONVERSATION_RECEIPT=PASS',
      'ACTION_STATE=SENT',
      'THREAD_STATE=AWAITING_CUSTOMER',
      'MESSAGE_GROUP_STATE=RESPONDED',
      'AMBIGUITY_COUNT=0',
  ...
  it('binds promotion authorization and canary evidence to exactly one target', () => {
    for (const marker of [
  ...
  it('allows unrelated post-canary changes but fails closed on critical engagement drift', () => {
    for (const marker of [
      'compare/${CANARY_SOURCE_SHA}...${GITHUB_SHA}',
      '.base_commit.sha == $canary',
      '.merge_base_commit.sha == $canary',
      'CRITICAL_DRIFT=',
      'src/instagram-engagement/',
      'src/ops/instagram-engagement-',
      'src/toca-managed-instagram-daemon\\.ts$',
      'src/config\\.ts$',
      '\\.github/workflows/instagram-engagement-',
      'instagram-engagement-comment-limited-promotion\\.yml$',
      'COMMENT_PROMOTION_CRITICAL_DRIFT_GATE=PASS',
    ]) {
      expect(workflow).toContain(marker);
    }
  });

  it('requires a healthy Direct-only LIMITED prestate and immutable current runtime', () => {
    for (const marker of [
