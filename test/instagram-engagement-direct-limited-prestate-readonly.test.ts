...
describe('Instagram engagement Direct LIMITED prestate read-only diagnostic', () => {
  it(
    'requires live owner-authored single-use authority and exact current main/runtime provenance',
    () => {
      for (const marker of [
  ...
  it(
    'uses GCP control-plane reads only and cannot mutate runtime, database, scheduler or provider state',
    () => {
  ...
  it(
    'publishes only sanitized state and consumes authority without authorizing a real canary',
    () => {
      expect(workflow).toContain(
