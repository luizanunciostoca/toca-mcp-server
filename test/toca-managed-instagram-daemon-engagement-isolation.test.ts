...
describe('TOCA managed Instagram daemon engagement isolation', () => {
  ...
  it('preserves scheduler watchdog wiring while isolating the publication component', () => {
    expect(daemonSource).toContain('watchdog: schedulerWatchdog');
    expect(daemonSource).toContain('onWatchdogSnapshot: (snapshot) => {');
    expect(daemonSource).toContain('lastSchedulerWatchdogSnapshot = snapshot;');
  });
});
