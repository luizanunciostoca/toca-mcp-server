...
describe('Instagram GCP execution-runtime readiness', () => {
  it('requires exact workflow-dispatch authorization bound to protected main SHA', () => {
  ...
  it('binds execution to an immutable application image and dedicated readiness entrypoint', () => {
    expect(workflow).toContain('docker build -t "$IMAGE_TAG" .');
  ...
  it('verifies the schema and privileges required by the historical execute path without exercising writes', () => {
    for (const required of [
      ...
      'provider_update',
      'audit_insert',
      'provider_tenant_default',
      'audit_tenant_default',
  ...
  it('keeps the owner-command gateway credentialless and scoped only to issue 869', () => {
    ...
    expect(gateway).toContain('instagram-gcp-publication-execution-readiness.yml');
    expect(gateway.match(/\/git\/ref\/heads\/main/g)).toHaveLength(2);
    expect(gateway).toContain('test "$live_main_sha" = "$AUTHORIZED_SOURCE_SHA"');
    expect(gateway).toContain('actions: write');
    expect(gateway).toContain('contents: read');
    expect(gateway).toContain('issues: read');
    expect(gateway).not.toContain('id-token: write');
