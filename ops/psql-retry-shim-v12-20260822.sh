...
# Preserve the V11 discovery corrections: include postgres as an application-DB
# candidate, exclude Cloud SQL's internal database, and select a candidate DB by
# schema_migrations existence without requiring table SELECT permission yet.
for i in "${!ARGS[@]}"; do
  if [[ "${ARGS[$i]}" == *"select datname from pg_database where datistemplate=false and datallowconn=true and datname <> 'postgres' order by datname"* ]]; then
    ...
    echo 'DR_LP_V12_DB_DISCOVERY_INCLUDE_POSTGRES_EXCLUDE_CLOUDSQLADMIN=PASS' >&2
    ...
    echo 'DR_LP_V12_APP_DB_SELECTION_BY_SCHEMA_MIGRATIONS=PASS' >&2
  fi
done
...
OWNER_FILE=/tmp/dr-v12-app-owner
OWNER_DB_FILE=/tmp/dr-v12-app-db
OWNER_PASSWORD_FILE=/tmp/dr-v12-app-owner-password
...
if [[ "$needs_owner_bootstrap" == true && ! -s "$OWNER_FILE" ]]; then
  ...
  gcloud --impersonate-service-account="$DR_SA_EMAIL" sql users list \
    --instance="$TARGET_INSTANCE" --project="$PROJECT_ID" --format=json > /tmp/dr-v12-users.json
  jq -e --arg owner "$APP_OWNER" 'any(.[]?; .name==$owner)' /tmp/dr-v12-users.json >/dev/null
  ...
  unset APP_PASSWORD
  echo 'DR_LP_V12_APP_TABLE_OWNER_AUTH=PASS clone_only=true grants_changed=false' >&2
fi
...
for attempt in $(seq 1 90); do
  ...
  if [[ "$rc" -eq 0 ]]; then
    if [[ "$attempt" -gt 1 ]]; then
      echo "DR_LP_V12_PSQL_TRANSIENT_RECOVERY=PASS attempt=${attempt}" >&2
    fi
    cat "$OUT"
    exit 0
  fi
...
if [[ -f /tmp/proxy.log ]]; then
  echo 'DR_LP_V12_PROXY_DIAGNOSTIC_BEGIN' >&2
  ...
  echo 'DR_LP_V12_PROXY_DIAGNOSTIC_END' >&2
fi
exit 91
