...
gcloud iam service-accounts create "$DR_SA_ID" --project="$PROJECT_ID" --display-name='TOCA Next temporary isolated DR v5' --description="Temporary DR identity for run ${GITHUB_RUN_ID} only" >/dev/null
...
gcloud iam roles create "$DR_ROLE_ID" --project="$PROJECT_ID" --title='TOCA Next isolated DR least privilege v5' --description="Temporary DR-only role for run ${GITHUB_RUN_ID}" --permissions="$PERMISSIONS" --stage=GA --quiet >/dev/null
gcloud iam roles describe "$DR_ROLE_ID" --project="$PROJECT_ID" --format=json >/tmp/dr-v5-role.json
[[ "$(jq -r '.includedPermissions[]' /tmp/dr-v5-role.json | sort)" == "$(tr ',' '\n' <<<"$PERMISSIONS" | sort)" ]]
...
for attempt in $(seq 1 60); do
  if gcloud --impersonate-service-account="$DR_SA_EMAIL" projects describe "$PROJECT_ID" --format='value(projectNumber)' >/tmp/dr-v5-project 2>/tmp/dr-v5-imp-error \
    && [[ "$(cat /tmp/dr-v5-project)" == "$PROJECT_NUMBER" ]] \
    && gcloud --impersonate-service-account="$DR_SA_EMAIL" sql instances describe "$SOURCE_INSTANCE" --project="$PROJECT_ID" --format='value(state)' >/tmp/dr-v5-source-state 2>>/tmp/dr-v5-imp-error \
    && [[ "$(cat /tmp/dr-v5-source-state)" == RUNNABLE ]]; then
    READY=1
    echo "DR_LP_V5_IMPERSONATION=PASS attempt=${attempt}"
    break
  fi
  sleep 10
done
[[ "$READY" == 1 ]] || { tail -n 20 /tmp/dr-v5-imp-error >&2 || true; exit 23; }

TOKEN="$(gcloud auth print-access-token --impersonate-service-account="$DR_SA_EMAIL")"
printf '%s' '{"permissions":["cloudsql.instances.clone","cloudsql.instances.connect","cloudsql.instances.create","cloudsql.instances.delete","cloudsql.instances.get","cloudsql.instances.update","cloudsql.operations.get","cloudsql.users.list","cloudsql.users.update","resourcemanager.projects.get","serviceusage.services.use"]}' >/tmp/dr-v5-test-perms-request.json
HTTP="$(curl --silent --show-error --output /tmp/dr-v5-test-perms.json --write-out '%{http_code}' -X POST -H "Authorization: Bearer ${TOKEN}" -H 'Content-Type: application/json' --data-binary @/tmp/dr-v5-test-perms-request.json "https://cloudresourcemanager.googleapis.com/v1/projects/${PROJECT_ID}:testIamPermissions")"
unset TOKEN
[[ "$HTTP" == 200 ]] || { cat /tmp/dr-v5-test-perms.json >&2; exit 24; }
for p in cloudsql.instances.clone cloudsql.instances.connect cloudsql.instances.create cloudsql.instances.delete cloudsql.instances.get cloudsql.instances.update cloudsql.operations.get cloudsql.users.list cloudsql.users.update resourcemanager.projects.get serviceusage.services.use; do
  jq -e --arg p "$p" '(.permissions // []) | index($p) != null' /tmp/dr-v5-test-perms.json >/dev/null || { echo "DR_LP_V5_MISSING_PERMISSION=$p" >&2; exit 25; }
done
echo 'DR_LP_V5_TEST_IAM_PERMISSIONS=PASS permissions=11'
echo 'DR_LP_V5_BOOTSTRAP=PASS'
