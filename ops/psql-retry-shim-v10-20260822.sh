...
for i in "${!ARGS[@]}"; do
  if [[ "${ARGS[$i]}" == *"select datname from pg_database where datistemplate=false and datallowconn=true and datname <> 'postgres' order by datname"* ]]; then
    ...
    echo 'DR_LP_V10_DB_DISCOVERY_INCLUDE_POSTGRES_EXCLUDE_CLOUDSQLADMIN=PASS' >&2
  fi
done
...
for attempt in $(seq 1 90); do
  ...
  if [[ "$rc" -eq 0 ]]; then
    if [[ "$attempt" -gt 1 ]]; then
      echo "DR_LP_V10_PSQL_TRANSIENT_RECOVERY=PASS attempt=${attempt}" >&2
    fi
    cat "$OUT"
    exit 0
  fi
...
if [[ -f /tmp/proxy.log ]]; then
  echo 'DR_LP_V10_PROXY_DIAGNOSTIC_BEGIN' >&2
  ...
  echo 'DR_LP_V10_PROXY_DIAGNOSTIC_END' >&2
fi
exit 91
